...
class ProposeTileInterfaceOrder(Step):
    ...
    def run(self, state_in: State, **_kwargs: str) -> tuple[ViewsUpdate, MetricsUpdate]:
        ...
        **_kwargs : str
        ...
        proposal = pins_by_side(ordered)
        design = self.config["DESIGN_NAME"]
        out = Path(self.step_dir) / f"{design}.{TILE_INTERFACE_ORDER_FORMAT.extension}"
        write_interface_order(proposal, out)

        applied_path = self.config[TILE_INTERFACE_ORDER_VARIABLE.name]
        applied = (
            None if applied_path is None else read_interface_order(Path(applied_path))
        )
        ...
        metrics: MetricsUpdate = {
            ...
            # The loop stops when a proposal repeats the order it was made
            # from, which only this step can see without re-reading the files.
            "fabulous__tile_interface__unchanged": int(proposal == applied),
        }
