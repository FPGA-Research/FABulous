...
def _bel_port_pairs(bel: Bel) -> list[tuple[str, str]]:
    """Bus a BEL's signals into the port pairs its instantiation takes.

    A vector port is driven by the concatenation of its per-bit signals, most
    significant first (e.g. `.A({A7, ..., A0})`); a scalar port is driven by its
    single signal. Internal ports come before external ones, which is the order
    they appear in the emitted instantiation.
    ...
    -------
    list[tuple[str, str]]
        The `(port name, signal)` pairs, keyed by the port name the BEL's own
        HDL declares.
    """
    port_dict: defaultdict[str, list[tuple[str, str]]] = defaultdict(list)
    for port_type, bel_ports in bel.ports_vectors.items():
        if port_type not in ("external", "internal"):
            continue
        for port_name, (_direction, width) in bel_ports.items():
            if width > 1:
                port_dict[port_name] = [
                    (f"{bel.prefix}{port_name}{i}", f"{i}") for i in range(width)
                ]
            else:
                port_dict[port_name] = [
    ...
    for port_name, signals in port_dict.items():
        if len(signals) > 1:
            signals.sort(key=lambda sig: int(sig[1]) if sig[1].isdigit() else -1)
            concatenated = ", ".join(name for name, _ in signals[::-1])
            pairs.append((port_name, f"{{{concatenated}}}"))
        else:
            pairs.append((port_name, signals[0][0]))
    return pairs
...
def generateTile(
    ...
    portList = (
        tile.ports_on(Side.NORTH)
        + tile.ports_on(Side.EAST)
        + tile.ports_on(Side.WEST)
        + tile.ports_on(Side.SOUTH)
    )
...
def _composite_wrapper_matrix_signals(
    ...
    sub-tile-qualified name (`{subtile}_{port}{k}`), and each such pair needs an
    intermediate signal bound to both the sub-tile and the wrapper-matrix instance.
...
def _generate_composite_tile(
    ...
    ports_around = tile.get_ports_around_tile()
    ...
    if config_bit_mode == ConfigBitMode.FRAME_BASED:
        for y, row in enumerate(tile_map):
            for x, sub in enumerate(row):
                # Holes own no config/clock terminals; emitting ports for them
                # leaks a phantom cell into the wrapper interface.
                if sub is None:
                    continue
    ...
    if not disable_user_clk:
        for y, row in enumerate(tile_map):
            for x, sub in enumerate(row):
                ...
                if sub is None:
                    continue
                # Top-first storage: the user clock chains south to north, like
                # FrameStrobe, so UserCLK enters from the south (y+1) and
                # UserCLKo exits north (y-1); each is a port only at that edge.
                if y - 1 < 0 or tile_map[y - 1][x] is None:
    ...
    if isinstance(writer, VHDLCodeGenerator):
        ...
        if tile.matrix_dir is not None:
    ...
    internal_connections = tile.get_internal_connections()
    ...
    # BEL pin signals bridging the wrapper BELs and the switch matrix, one per bit
    bel_pin_signals = [
        name for bel in tile.bels for name in (*bel.inputs, *bel.outputs)
    ]
    if bel_pin_signals:
    ...
    for y, row in enumerate(tile_map):
        for x, sub in enumerate(row):
            # A hole owns no terminals, so declaring wires for it would leak a
            # phantom cell into the wrapper.
            if sub is None:
                continue
            ...
            # the north (y-1) to receive it, and the user clock is enabled.
            if (
            ...
            # FrameData runs west to east, so FrameData_O is an internal wire
            # only when a tile to the east (x+1) consumes it; otherwise it is a
            # boundary port. Keying this on the west neighbour leaves the real
            # producer's bus undeclared, and Verilog silently truncates it to a
            # one-bit implicit net.
    ...
    for y, row in enumerate(tile_map):
        for x, sub in enumerate(row):
            ...
            north_port = [i.name for i in sub.ports_on(Side.SOUTH, IO.INPUT)]
            # Top-first storage: northbound wires end on this tile's south side,
            # so they are driven by the tile to the south (y+1).
            if 0 <= y + 1 < len(tile_map) and tile_map[y + 1][x] is not None:
                for p in tile_map[y + 1][x].ports_on(Side.NORTH, IO.OUTPUT):
                    north_input.append(f"Tile_X{x}Y{y + 1}_{p.name}")
            else:
                for p in sub.ports_on(Side.SOUTH, IO.INPUT):
            ...
            east_port = [i.name for i in sub.ports_on(Side.WEST, IO.INPUT)]
            if 0 <= x - 1 < len(tile_map[0]) and tile_map[y][x - 1] is not None:
                for p in tile_map[y][x - 1].ports_on(Side.EAST, IO.OUTPUT):
                    east_input.append(f"Tile_X{x - 1}Y{y}_{p.name}")
            else:
                for p in sub.ports_on(Side.WEST, IO.INPUT):
            ...
            south_port = [i.name for i in sub.ports_on(Side.NORTH, IO.INPUT)]
            # Top-first storage: southbound wires end on this tile's north side,
            # so they are driven by the tile to the north (y-1).
            if 0 <= y - 1 < len(tile_map) and tile_map[y - 1][x] is not None:
                for p in tile_map[y - 1][x].ports_on(Side.SOUTH, IO.OUTPUT):
                    south_input.append(f"Tile_X{x}Y{y - 1}_{p.name}")
            else:
                for p in sub.ports_on(Side.NORTH, IO.INPUT):
            ...
            west_port = [i.name for i in sub.ports_on(Side.EAST, IO.INPUT)]
            if 0 <= x + 1 < len(tile_map[0]) and tile_map[y][x + 1] is not None:
                for p in tile_map[y][x + 1].ports_on(Side.WEST, IO.OUTPUT):
                    west_input.append(f"Tile_X{x + 1}Y{y}_{p.name}")
            else:
                for p in sub.ports_on(Side.EAST, IO.INPUT):
                    west_input.append(f"Tile_X{x}Y{y}_{p.name}")

            ports_pairs += list(zip(west_port, west_input, strict=False))

            for p in (
                sub.ports_on(Side.NORTH, IO.OUTPUT)
                + sub.ports_on(Side.EAST, IO.OUTPUT)
                + sub.ports_on(Side.SOUTH, IO.OUTPUT)
                + sub.ports_on(Side.WEST, IO.OUTPUT)
            ):
            ...
            for port_name in wrapper_ports_by_sub.get(sub.name, []):
                ports_pairs.append((port_name, f"{sub.name}_{port_name}"))

            # add clock to tile
            if not disable_user_clk:
                # Top-first storage: UserCLK is chained from the south tile (y+1).
                if 0 <= y + 1 < len(tile_map) and tile_map[y + 1][x] is not None:
    ...
    sm_config_bits = tile.switch_matrix.no_config_bits
    if tile.matrix_dir is not None:
        sm_ports_pairs = []
        ...
        for bel in tile.bels:
            for name in bel.inputs:
                sm_ports_pairs.append((name, name))
        for bel in tile.bels:
    ...
    for bel in tile.bels:
        # The signals {prefix}{port}{i} bussed here are the wrapper SM outputs.
        bel_ports_pairs = _bel_port_pairs(bel)
        if not disable_user_clk and bel.withUserCLK:
            # The wrapper carries no clock port of its own; the BEL shares the master
            # cell's clock net. Top-first storage: the master's clock is the buffered
            # copy chained from the south cell (y+1), or its own clock input.
            if 0 <= my + 1 < len(tile_map) and tile_map[my + 1][mx] is not None:
        ...
        if bel.configBit > 0 and config_bit_mode == ConfigBitMode.FRAME_BASED:
            bel_ports_pairs.append(
                (
                    "ConfigBits",
                    f"ST_ConfigBits["
                    f"{bel_config_offset + bel.configBit}-1:{bel_config_offset}]",
                )
            )
        bel_config_offset += bel.configBit
        writer.addInstantiation(
            compName=bel.name,
