...
class ConfigMemFrame:
    ...
    `used_bits_mask` reads left to right from `FrameData[width - 1]` down to
    `FrameData[0]`, and the n-th `1` binds `config_bit_ranges[n]`, so a frame
    ...
    @property
    def bits_used_in_frame(self) -> int:
...
class ConfigMem:
    ...
    memory carries the grid it belongs to, so one built against the wrong
    `FrameBitsPerRow` or `MaxFramesPerCol` cannot exist.
    ...
    def __post_init__(self) -> None:
        ...
        ------
        ValueError
            If the frames are not 0 .. n-1 in order, if there are not
            `max_frames_per_col` of them, if one is not `frame_bits_per_row`
            wide, or if a configuration bit is allocated twice.
        """
        indices = [frame.frame_index for frame in self.frames]
        if indices != list(range(len(self.frames))):
            raise ValueError(
                f"{self} frames must be indexed 0 .. n-1 in order, got {indices}"
            )
        ...
        for frame in self.frames:
            if frame.width != self.frame_bits_per_row:
                raise ValueError(
                    f"{self} has a too long or short bitmask for frame : "
                    f"{frame.frame_name}"
                )
    ...
    def __str__(self) -> str:
        """Name the memory in a message, by its file where it came from one."""
        if self.source is None:
            return (
                f"Configuration memory of {self.max_frames_per_col} x "
                f"{self.frame_bits_per_row} crosspoints"
            )
    ...
    def used_frames(self) -> tuple[ConfigMemFrame, ...]:
        ...
        return tuple(frame for frame in self.frames if frame.bits_used_in_frame > 0)

    @property
    def config_bits(self) -> int:
        """Number of bits the memory holds."""
        return sum(frame.bits_used_in_frame for frame in self.frames)
    ...
    def from_csv(
        ...
        `bits_used_in_frame` is ignored and recomputed from the mask. A range
        is `hi:lo` or `lo:hi` for one run, `x;y;z` for a list, one integer, or
        NULL for an empty frame.
    ...
    def default(
        ...
        return cls(tuple(frames), frame_bits_per_row, max_frames_per_col)
    ...
    def for_tile(
        ...
        *,
        ...
        """Read the memory a tile implements, or lay out the bits it will get.

        A tile whose `<tile>_ConfigMem.csv` has not been written yet holds the
        enumerated default, which is the mapping generating that file produces,
        so the model says the same thing before and after generation.

        Parameters
        ----------
        path : Path
            The tile's `<tile>_ConfigMem.csv`, which need not exist.
        config_bits : int
            The number of bits the tile holds.
        frame_bits_per_row : int
        ...
        -------
        Self
            The memory, from the file where there is one.

        Raises
        ------
        ValueError
            If the file holds a different number of bits than the tile has.
        """
        if not path.is_file():
            return cls.default(
                config_bits,
        ...
        if memory.config_bits != config_bits:
            raise ValueError(
                f"{memory} has a bitmask mismatch; bitmask has in total "
                f"{memory.config_bits} 1-values for {config_bits} bits."
            )
        return memory
    ...
    def rebuilt_with(self, bit_at: Mapping[Crosspoint, int]) -> Self:
        ...
        return type(self)(
            tuple(frames), self.frame_bits_per_row, self.max_frames_per_col
        )


EMPTY_CONFIG_MEM = ConfigMem((), 0, 0)
"""A memory of no frames, which is what a tile holds before one is read."""
