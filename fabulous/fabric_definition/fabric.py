...
class Fabric:
    ...
    def _composite_matches_grid(
        ...
        for ly, st_row in enumerate(rows):
            fy = base_fy + composite.fabric_dy(ly)
    ...
    def find_tile_positions(self, tile: Tile) -> list[tuple[int, int]] | None:
        ...
        if tile.is_composite:
            if tile.tile_map is None:
                return None
            for base_fx, base_fy in self.find_composite_placement_origins(tile):
                # Report every covered grid cell (every non-None sub-tile).
                for dx, fdy, _ in tile.iter_cells_fabric():
                    positions.append((base_fx + dx, base_fy + fdy))
        else:
            for y, row in enumerate(self.tile):
    ...
    def find_composite_placement_origins(
        ...
        scan as `find_tile_positions`, but instead of every covered cell only
    ...
    def composite_master_positions(self, composite: Tile) -> list[tuple[int, int]]:
        ...
        cell, whose top-first `get_master_offset` index is flipped into the
        bottom-first fabric grid. One coordinate is emitted per placement, so
        contiguous repeats of the same composite each get their own.
        ...
        master_dy = composite.fabric_dy(my)
        return [
            (base_fx + mx, base_fy + master_dy)
            for base_fx, base_fy in self.find_composite_placement_origins(composite)
        ]
    ...
    def get_all_unique_tiles(self) -> list[Tile]:
        ...
        return [
            tile
            for tile in self.tileDic.values()
            if tile.is_composite or not tile.part_of_composite
        ]
