...
def script_cmd(
    ...
    """Execute a script file as a FABulous or TCL script.

    The script type defaults to TCL and can be set with --type.
    ...
    # Execute the script using the interpreter for the selected type.
    if script_type == ScriptType.FABULOUS:
...
def convert_legacy_args_with_deprecation_warning() -> None:
    ...
    parser.add_argument(
        "-w",
        "--writer",
        choices=[HDLType.VERILOG, HDLType.VHDL],
        type=lambda s: HDLType(s.lower()),
        help=(
    ...
    if args.createProject:
        ...
        create_project_cmd(project_dir, args.writer)
        sys.exit(0)
