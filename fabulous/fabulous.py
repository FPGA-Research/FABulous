...
def plugins_callback(
    ...
    `-m/--plugin` is accepted here too, so a plugin still under development can
    be inspected the same way `start` would load it.
    """
    ctx.obj = PluginManager.create(
        extra_plugins=plugin or (), skip_broken=skip_broken_plugins
    )
