...
def generateUserDesignTopWrapper(
    ...
    user_design = Bel.from_hdl(user_design_path, "")
    ...
    for x in range(fabric.numberOfColumns):
        ...
        for y in range(fabric.numberOfRows - 1, -1, -1):
            ...
            for i, bel in enumerate(
                ...
                if not bel.external_input and not bel.external_output:
                    logger.info(
                    ...
                    continue
                if (
                    len([p.name for p in bel.inputs] + [p.name for p in bel.outputs])
                    == 0
                ):
                    logger.info(
                ...
                if bel.name not in bel_count:
                    ...
                    bel_inputs[bel.name] = [
                        port.name.removeprefix(bel.prefix) for port in bel.inputs
                    ]
                    bel_outputs[bel.name] = [
                        port.name.removeprefix(bel.prefix) for port in bel.outputs
                    ]
                else:
                ...
                for port in bel.inputs + bel.outputs:
                    port_name = port.name.removeprefix(bel.prefix)
                    if first:
                        first = False
                    else:
                        belstr += ", "
    ...
    if (
        ...
        if user_design.language == "vhdl":
            ...
            for port in user_design.inputs + user_design.outputs:
                ...
                if port.name in ["clk", "CLK"]:
                    user_design_inst += f".{port.name}(clk) "
                else:
                    user_design_inst += f".{port.name}() "
        else:  # verilog
            first = True
