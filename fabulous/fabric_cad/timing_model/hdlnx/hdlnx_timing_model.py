...
class HdlnxTimingModel(VerilogGateLevelTimingGraph):
    ...
    def _check_config(self):
        ...
        if isinstance(self.config["verilog_files"], list):
            ...
            if not self.config["verilog_files"].exists():
                raise FileNotFoundError(
                    f"Verilog file not found: {self.config['verilog_files']}"
                )
            if self.config["verilog_files"].stat().st_size == 0:
                raise ValueError(
                    f"Verilog file is empty: {self.config['verilog_files']}"
                )

        if not isinstance(self.config["is_gate_level"], bool):
            raise TypeError("is_gate_level must be a boolean value.")

        if self.config["is_gate_level"] == True and not isinstance(
        ...
        if (
            ...
            and self.config["spef_files"] is not None
        ):
            raise ValueError(
                "SDF back-annotation via SPEF files is only supported for gate-level netlists."
    ...
    def _apply_config(self):
        ...
        if self.config["is_gate_level"]:
            ...
            super().__init__(
                verilog_netlist=self.config["verilog_files"],
                liberty_files=self.config["liberty_files"],
                top_name=self.config["top_name"],
                sta_executable=self.config["sta_executable"],
                sta_program=self.config["sta_program"],
                spef_files=self.config["spef_files"],
                delay_type_str=self.config["delay_type_str"],
                hier_sep=self.config["hier_sep"],
                debug=self.config["debug"],
            )
        else:
            if self.config["synth_program"] in self.registered_synth_tools:
                tmp_gate_level_netlist_path: Path = self.registered_synth_tools[
                    self.config["synth_program"]
                ]
            else:
                raise NotImplementedError(
                    f"Synthesis tool '{self.config['synth_program']}' is not supported."
                )

            # Initialize the parent VerilogGateLevelTimingGraph class with the generated gate-level netlist
            super().__init__(
                verilog_netlist=tmp_gate_level_netlist_path,
                liberty_files=self.config["liberty_files"],
                top_name=self.config["top_name"],
                sta_executable=self.config["sta_executable"],
                sta_program=self.config["sta_program"],
    ...
    def _generate_gate_level_netlist_yosys(self) -> Path:
        ...
        if (self.config["tiehi_cell_and_port"] is None) ^ (
            self.config["tielo_cell_and_port"] is None
        ):
            raise ValueError(
        ...
        if isinstance(self.config["verilog_files"], Path):
            synth_tcl_script += (
                f"read_verilog -overwrite -sv {self.config['verilog_files']}\n"
            )
        else:
            for vf in self.config["verilog_files"]:
        ...
        synth_tcl_script += f"clockgate -liberty  {
            self.config['liberty_files'][0]
            if isinstance(self.config['liberty_files'], list)
            else self.config['liberty_files']
        }\n"
        ...
        if (
            self.config["tiehi_cell_and_port"] is not None
            and self.config["tielo_cell_and_port"] is not None
        ):
            synth_tcl_script += (
        ...
        self._call_external(
            ...
            stdin_data=synth_tcl_script.format(
                synth_output_file=path, synth_output_json_file=json_path
            ),
            debug=self.config["debug"],
            args=["-C"],
        )
        ...
        with open(json_path, "r") as f:
            ...
            if self.json_netlist is None or len(self.json_netlist) == 0:
                raise RuntimeError(
                    "Failed to generate JSON netlist using Yosys. No content in JSON netlist file."
                )
        if json_path is None:
            raise RuntimeError(
                "Failed to generate JSON netlist using Yosys. No JSON netlist file created."
            )
        os.remove(json_path)
    ...
    def get_netlist_as_json(self) -> str:
        ...
        if self.json_netlist is None:
            raise RuntimeError(
                "Netlist JSON representation is not available. This depends on the tool flow used."
            )
