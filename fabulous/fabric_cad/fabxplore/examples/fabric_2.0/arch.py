...
Fabric 2.0 Achitecture implementation.
"""
...
class FabulousArchitecture(ArchitectureSynthesizer):
    ...
    def __init__(self, debug: bool = False) -> None:
        ...
        self.my_root: Path = self.x_root / "examples" / "fabric_2.0"
    ...
    def synth_fabulous(self, config: FabulousArchitectureConfig) -> None:
        """Run Fabric 2.0 synth script, but converting FFs."""
        # REGFILE MAP
        ...
        self.design_auto_techmap(
            cell_type="$macc_v2",
            techmap_file=Path(f"{self.my_root / 'arch_rtl' / 'dsp_map.v'}"),
        )
        ...
        self.design.run_pass(
            f"techmap -map {self.my_root / 'arch_rtl' / 'custom_map.v'}"
        )
        self.design_connect_clock(cell_type="LUTFF", clock_port="CLK")
        self.design.run_pass("clean")

        # ABC costfunction for MUX2,3,8LUT
        self.design.run_pass("abc9 -luts 69,62,56,58,60,128,188,256")

        # MAP to MUX8LUT
        self.design_decompose_lut_pass(
            source_lut_widths=[8],
            leaf_lut_width=5,
            mux_verilog_path=self.my_root / "arch_rtl" / "FABULOUS_MUX8.v",
        ...
        self.design_decompose_lut_pass(
            source_lut_widths=[7],
            leaf_lut_width=5,
            mux_verilog_path=self.my_root / "arch_rtl" / "FABULOUS_MUX4.v",
            mux_dependency_paths=[self.project_context.models_pack],
        ...
        self.design_decompose_lut_pass(
            source_lut_widths=[6],
            leaf_lut_width=5,
            mux_verilog_path=self.my_root / "arch_rtl" / "FABULOUS_MUX2.v",
            mux_dependency_paths=[self.project_context.models_pack],
        ...
        self.design.write_verilog_path(
            config.user_design_out_dir / f"before_multimap_{config.top_module}.v",
            include_attributes=True,
        )
        self.design.write_json_path(
            config.user_design_out_dir / f"before_multimap_{config.top_module}.json"
        )
        input("Press Enter to continue to multi-mapping stage...")

        # Generic SAT-MAPPING
        # Consider 3,4 cuts with boundary constraints
        ...
        # Map here the rest of the LUTs especially the ones
        # produced by the LUT combinator pass.
        self.design_morph_tile_pass(
            tile_verilog_path=self.my_root / "arch_rtl" / "FLUT5_1P_2PS.v",
            tile_top_name="FLUT5_1P_2PS",
            tile_inputs=["I0", "I1", "I2", "A0", "B0", "S", "Ci"],
    ...
    def build_tile(self, config: FabulousArchitectureConfig) -> None:
        ...
        # Generate switch matrix pattern image for LUT5F
        # after adding long wire connections.

        self.write_switch_matrix_pattern_image(
            "LUT5F",
            config.user_design_out_dir / "lut5f_swm_after.png",
            mode="binary",
            pixel_size=4,
            grid_every=16,
        )
        ...
        self.fpga_model.nextpnr_route(
            top_name=config.top_module,
            json_path=config.user_design_out_dir / f"{config.top_module}.json",
    ...
    def synthesize(
        ...
        if syth_coarse_only and synth_enable:
    ...
    def dse_flow(
        ...
        self.synthesize(
            config,
            syth_coarse_only=syth_coarse_only,
            pnr=pnr,
            synth_enable=synth_enable,
        )
        self.clear_flow()
