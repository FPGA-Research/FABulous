    ...
    for input_fanin, output_fanin in _fanin_candidates(routing):
        ...
        if matrix_config_budget is None or matrix_bits <= matrix_config_budget:
            if (input_fanin, output_fanin) != (
                ...
                result = result.model_copy(update={"warnings": tuple(warnings)})
            return result
...
def _fanin_candidates(routing: BaselineRouting) -> list[tuple[int, int]]:
    ...
    input_fanin = routing.input_fanin
    output_fanin = routing.output_fanin
    while True:
        candidates.append((input_fanin, output_fanin))
        if output_fanin > routing.min_output_fanin:
            output_fanin -= 1
        elif input_fanin > routing.min_input_fanin:
            input_fanin -= 1
            output_fanin = routing.output_fanin
        else:
            break
