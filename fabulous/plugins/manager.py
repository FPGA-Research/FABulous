...
A directory plugin, or a session plugin given as a path, is named by its
directory, which is not a namespace FABulous controls, so entering it
unprefixed would let a plugin directory called `yaml` replace the real `yaml`
...
class PluginManager:
    """Owns plugin discovery, lifecycle hooks, provider registries and management."""
    ...
    def _plugin_version(self, name: str) -> str:
        ...
        Built-in plugins are versioned with FABulous itself, and entry-point
        plugins with their distribution. Directory and session plugins are
        loaded from bare paths, but the path may still belong to
    ...
    def _fold_registry(
        ...
        Each provider hook returns one list of providers per plugin, and each
        provider claims a unique key. A second plugin claiming a taken key is a
        conflict.
    ...
    def make_writer(self, hdl_type: HDLType) -> CodeGenerator:
        ...
        Writers are resolved only here. The method selects the registered
        provider and constructs the generator. A provider needing
    ...
    def install(cls, spec: str) -> tuple[bool, str]:
        ...
        Installing reads no plugin state and needs no discovered manager, so a
        plugin already broken on this machine cannot block installing another.
    ...
    def _register_external(self, name: str, load: Callable[[], object]) -> None:
        ...
        Directory, entry-point and session plugins are untrusted input, so each
        must declare a compatible contract version through a module-level
        `FABULOUS_PLUGIN_API` attribute. Any load, version or registration
        failure aborts unless `skip_broken` downgrades it to a warning.
    ...
    def _register_builtins(self) -> None:
        """Register the built-in plugins, which are always present."""
    ...
    def core_only(cls) -> Self:
        ...
        -------
        Self
            A manager with the built-in plugins registered and registries built.
        """
        manager = cls()
    ...
    def create(
        ...
        """Build a manager with every plugin source discovered.
        ...
        -------
        Self
            The populated manager, with every plugin source discovered and the
        ...
        # Sorted so a registration clash is reported against the same pair of
