...
def fabulous_register_gds_flows() -> list[GdsFlowProvider]:
    """Return `list[GdsFlowProvider]` keyed by the name used in `meta.flow`.

    Returns
    -------
    list[GdsFlowProvider]
        Hardening flows contributed by the plugin.
    """
...
class FABulousHookRelay(Protocol):
    ...
    def fabulous_register_gds_flows(self) -> list[list[GdsFlowProvider]]:
        """See `fabulous_register_gds_flows`."""
