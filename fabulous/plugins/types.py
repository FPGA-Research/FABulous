...
annotations naming the fabric model and the timing interface are deferred, so
importing a descriptor does not import the generator stack behind them.
"""
...
class PnRModelProvider:
    ...
    as a mapping of file name to file content, so each backend chooses its
    file names, file count and timing encoding; the caller only writes the
    bytes out.
