...
class FABulous_API:
    ...
    def genTileMacro(
        ...
        *,
        ...
        `flow` names a registered flow to run instead of the one the configs
        select; `None` honours `meta.flow`.
        """
        logger.info(f"PDK root: {pdk_root}")
        logger.info(f"PDK: {pdk}")
        logger.info(f"Output folder: {out_folder.resolve()}")
        tile_flow_cls = self.plugin_manager.resolve_gds_flow(
            FABulousTileVHDLMacroFlow
            if isinstance(self.writer, VHDLCodeGenerator)
            else FABulousTileVerilogMacroFlow,
            [base_config_path, config_override_path],
    ...
    def fabric_stitching(
        ...
        **custom_config_overrides: dict,
        ...
        flow : str | None
            Name of a registered flow to run instead of the one the configs
            select. Defaults to None, which honours `meta.flow`.
        **custom_config_overrides : dict
        ...
        fabric_flow_cls = self.plugin_manager.resolve_gds_flow(
            FABulousFabricVHDLMacroFlow
            if isinstance(self.writer, VHDLCodeGenerator)
    ...
    def full_fabric_automation(
        ...
        **config_overrides: dict,
        ...
        automation_flow_cls = self.plugin_manager.resolve_gds_flow(
            FABulousFabricOptimisationFlow,
            [base_config_path, config_override_path],
            flow_override=flow,
        )
        flow = automation_flow_cls(
            configs,
