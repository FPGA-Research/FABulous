"""Yosys tool wrapper: converts Verilog to JSON and synthesizes RTL to gates.

Used as a singleton through classmethods (`YosysTool.convert_to_json(...)`,
`YosysTool.synthesize(...)`, `YosysTool.run(...)`); never instantiated. The
gate-level netlist `synthesize` produces is later analyzed by an STA tool (e.g.
OpenSTA) to generate an SDF file.
"""
...
class YosysTool(Tool):
    ...
    Converts Verilog into Yosys's JSON netlist format and synthesizes Verilog RTL
    into a gate-level netlist, supporting techmapping, tie-high/low cell mapping,
    and buffer insertion. Used as a singleton: call the classmethods directly,
    never instantiate.
    """
