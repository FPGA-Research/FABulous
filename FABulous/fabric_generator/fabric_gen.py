...
class FabricGenerator:
    ...
    def genIOBel(self, gen_ios: list[Gen_IO], bel_name: str, overwrite: bool = True) -> Bel | None:
        """
        ...
        ------
        ValueError
            - If a wrong language is specified.
        ...
        if len(gen_ios) == 0:
            logger.info(f"No generative IOs, skipping genIOBel generation")
            return None

        language = "vhdl" if isinstance(self.writer, VHDLWriter) else "verilog"
        outFile = Path(self.writer.outFileName)
        if outFile.exists():
            if overwrite:
                outFile.unlink()
                logger.info(f"Overwriting existing Gen_IO BEL file: {outFile}")
            else:
                logger.info(f"Return existing Gen_IO BEL file: {outFile}")
                return parseBelFile(outFile, "", language)
        ...
        self.writer.addComment(f"Generative IO BEL for {bel_name}", onNewLine=True)
        self.writer.addComment(
            "This is a generated file, please don't edit!", onNewLine=True
        )
        self.writer.addNewLine()

        if configBits > 0:
            self.writer.addBelMapAttribute(belMap)

        self.writer.addHeader(f"{bel_name}")
        self.writer.addParameterStart(indentLevel=1)
        self.writer.addParameter("NoConfigBits", "integer", configBits, indentLevel=2)
        self.writer.addParameterEnd(indentLevel=1)
        self.writer.addPortStart(indentLevel=1)
                ...
                if gio.IO == IO.INPUT:
                    ...
                    if gio.clockedComb:  # clocked combinatorial also has a Q signal
                        ...
                        externalPorts.append(
                            (f"{gio.prefix}_Q_top{j}", IO.OUTPUT, True)
                        )
                    ...
                    if not gio.configAccess:
                        ...
                        if gio.clockedComb:
                            ...
                            internalPorts.append(
                                (f"{gio.prefix}_Q{j}", IO.OUTPUT, True)
                            )
        ...
        for port, direction, reg in internalPorts:
            self.writer.addPortScalar(port, direction, reg, indentLevel=2)

        for port, direction, reg in externalPorts:
            self.writer.addPortScalar(port, direction, reg, "EXTERNAL", indentLevel=2)
        ...
        if isinstance(self.writer, VHDLWriter):
            self.writer.addComment("GLOBAL", True, indentLevel=2)
            if configBits > 0:
                self.writer.addPortVector(
                    "ConfigBits", IO.INPUT, "NoConfigBits-1", indentLevel=2
                )
            ...
            if configBits > 0:
                self.writer.addPortVector(
                    "ConfigBits", IO.INPUT, "NoConfigBits -1", "GLOBAL", indentLevel=2
                )

        self.writer.addPortEnd(indentLevel=1)
        self.writer.addHeaderEnd(f"{bel_name}")
        self.writer.addNewLine()
        # declare architecture
        self.writer.addDesignDescriptionStart(f"{bel_name}")
        self.writer.addLogicStart()

        # gen_io config bit access
        if any(gio.configAccess for gio in gen_ios):
            self.writer.addNewLine()
            self.writer.addComment("gen_io config access", onNewLine=True)
            ...
            if gio.clocked:
                ...
                if gio.clockedMux:
                    for i in range(gio.pins):
                        ...
                        self.writer.addConnectionScalar(reg, True)
                        self.writer.addRegister(
                            reg,
                            source,
                            inverted=gio.inverted,
        ...
        bel: Bel
        if isinstance(self.writer, VHDLWriter):
            bel = parseBelFile(self.writer.outFileName, "", "vhdl")
        else:  # Verilog
            bel = parseBelFile(self.writer.outFileName, "", "verilog")

        prims_file = (
            Path(os.getenv("FAB_PROJ_DIR", ".")) / "user_design" / "custom_prims.v"
        )
