...
class FABulousShell(Cmd):
    ...
    def do_synthesis(self, args):
        """Runs Yosys using Nextpnr JSON backend to synthesise the Verilog or VHDL design specified
        ...
        if path.suffix == ".v": 
            runCmd = [
                ...
                f"{self.projectDir}/{parent}/{hdlFileName}",
                f"{self.projectDir}/{parent}/top_wrapper.v",
            ]
        elif path.suffix == ".vhdl" or path.suffix == ".vhd":
            runCmd = [
                "yosys",
                "-m",
                "ghdl",
                "-p",
                f"ghdl {self.projectDir}/{parent}/{hdlFileName} -e top;"
                f"read_verilog {self.projectDir}/{parent}/top_wrapper.v;"
                f"synth_fabulous -top top_wrapper -json {self.projectDir}/{parent}/{json_file}",
            ]
        else:
            logger.error("Invalid HDL file provided. The extension should be .v, .vhdl, .vhd")
            return

        try:
            logger.info(f"Running Yosys with the command: {' '.join(runCmd)}")
