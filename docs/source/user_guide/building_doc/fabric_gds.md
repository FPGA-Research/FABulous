...
Installing an order also rewrites every tile's `Tile/<name>/<name>_io_pin_order.yaml`, so the file a tile is hardened from already leads each border with the ordered pins, and the same rewrite happens whenever one of those files is regenerated by `gen_io_pin_config` or by `gen_tile_macro`. An order copied into `Tile/include/tile_interface_order.yaml` by hand reaches the pin YAMLs through
