...
- `fabric_metadata.yaml` -- machine-readable fabric inventory (geometry, tile and primitive counts, grid, bitstream size; generated by `gen_fabric_metadata`)
- `<fabric>_defines.v` / `<fabric>_pkg.sv` / `<fabric>_pkg.vhdl` -- HDL constants derived from the same inventory (also from `gen_fabric_metadata`)
...
## Generate fabric inventory metadata

Export a machine-readable description of the loaded fabric (size, configuration
parameters, tile and primitive counts, grid layout, and full bitstream length).
YAML is the canonical document; Verilog defines, a SystemVerilog package, and a
VHDL package are optional views of the same data for firmware and testbenches.

```bash
FABulous> gen_fabric_metadata
FABulous> gen_fabric_metadata --format yaml
FABulous> gen_fabric_metadata --format verilog --format systemverilog --format vhdl
```

Outputs are written under `.FABulous/` by default (`fabric_metadata.yaml`,
`<fabric>_defines.v`, `<fabric>_pkg.sv`, `<fabric>_pkg.vhdl`). This step also
runs as part of `run_fab`.
