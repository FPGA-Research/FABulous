...
- `--outfile <path>` — Where that template is written. Passing it without
    `--emit-config-template` is an error.
...
# The manager runs discovery and builds the code generator the API uses.
...
# The interface returns the artifact set; the caller chooses the output directory.
