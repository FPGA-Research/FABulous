...
A hook implementation that raises aborts the session with an error that names
your plugin. Under
...
are embedded, so a backend may put them inline, in a sidecar file or in a binary
database.
...
anywhere with full typing through `from_context`, which reads the settings
singleton from `get_context()`:
