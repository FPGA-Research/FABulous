"""Tests for what `TileAreaOptimisation` adds when a placement optimisation is on.
...
def opt_step(
    ...
    step.loop = PlacementOptLoop(
        config, growth_cap=TileAreaOptimisation.max_iterations, no_opt=True
    )
    return step


def test_the_optimisation_steps_sit_around_the_loop_body() -> None:
    steps = TileAreaOptimisation.Steps
    ...
    assert CONFIG_MEM_FORMAT in TileAreaOptimisation.outputs
    assert TILE_INTERFACE_ORDER_FORMAT in TileAreaOptimisation.outputs
    assert TileAreaOptimisation.propagate_exceptions == (GDSFlowError,)


def test_run_sizes_the_body_by_the_growth_cap_plus_the_budget(
    opt_step: TileAreaOptimisation, mock_state: State, mocker: MockerFixture
) -> None:
    run = mocker.patch(
    ...
    assert opt_step.max_iterations == TileAreaOptimisation.max_iterations + BUDGET
    run.assert_called_once()


def test_the_body_stops_after_the_proposals_when_the_iteration_does_not_route(
    opt_step: TileAreaOptimisation, mock_state: State
) -> None:
...
class TestBalanceShrink:
    @pytest.fixture
    def balance_step(self, opt_step: TileAreaOptimisation) -> TileAreaOptimisation:
    ...
    def test_the_larger_axis_shrinks_after_a_clean_iteration(
        self, balance_step: TileAreaOptimisation
    ) -> None:
    ...
    def test_the_pin_floor_ends_the_loop(
        self, balance_step: TileAreaOptimisation, mock_state: State
    ) -> None:
    ...
    def test_pre_iteration_shrinks_the_die_once_a_clean_run_exists(
        self, balance_step: TileAreaOptimisation, mock_state: State, tmp_path: Path
    ) -> None:
...
def test_post_loop_exports_the_winners_inputs_as_views(
    opt_step: TileAreaOptimisation, files: dict[str, Path], mock_state: State
) -> None:
