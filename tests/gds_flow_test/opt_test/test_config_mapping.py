...
def _bunch_pins_in_a_corner(raw: dict) -> None:
    ...
    for name, coord in (("FrameData", "y"), ("FrameStrobe", "x")):
        for index, value in ((0, 90), (1, 95)):
            for port in (f"{name}[{index}]", f"{name}_O[{index}]"):
                raw["pins"][port][coord] = value


def _reverse_pin_order(raw: dict) -> None:
    for name, coord in (("FrameData", "y"), ("FrameStrobe", "x")):
        for port in (f"{name}[0]", f"{name}_O[0]", f"{name}[1]", f"{name}_O[1]"):
            raw["pins"][port][coord] = 100 - raw["pins"][port][coord]
...
def test_frame_grid_spreads_the_trunks_over_the_die(
    ...
    grid = frame_grid(_placement(tmp_path, raw), LINES)
...
def test_frame_grid_crosspoints_run_frame_then_data_bit(placement: Placement) -> None:
    assert frame_grid(placement, LINES).crosspoints() == [
        Crosspoint(0, 0),
        Crosspoint(0, 1),
        Crosspoint(1, 0),
        Crosspoint(1, 1),
...
def test_the_proposal_is_a_memory_of_the_same_grid(placement: Placement) -> None:
    proposal, _ = solve_config_mapping(placement, CURRENT, LINES)
...
def test_mapping_rejects_a_latch_the_csv_does_not_allocate(
    ...
    memory = ConfigMem((ConfigMemFrame("frame0", 0, 2, {1: 1, 0: 0}),), 2, 1)
...
def test_reconnections_give_every_latch_its_target_port_nets() -> None:
    ...
    assert reconnections(pins, bit_at, proposal, LINES) == {
...
def test_reconnections_reject_a_latch_on_an_unallocated_crosspoint() -> None:
    ...
    with pytest.raises(GDSFlowError, match="does not allocate"):
        reconnections(pins, {}, proposal, LINES)
