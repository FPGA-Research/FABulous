...
def test_get_raw_verilog_netlist_data(vg):
    assert vg.get_raw_verilog_netlist_data() == TEST_NETLIST

def test_find_verilog_modules_regex_all(vg):
    assert vg.find_verilog_modules_regex(r".*") == ["LeafWrap", "Mid", "Top"]

def test_find_verilog_modules_regex_filtered(vg):
    assert vg.find_verilog_modules_regex(r"^L") == ["LeafWrap"]

def test_find_verilog_modules_regex_no_match(vg):
    assert vg.find_verilog_modules_regex(r"^XYZ$") == []

def test_find_instance_paths_by_regex_matches_recursive_paths(vg):
...
def test_find_instance_paths_by_regex_with_filter(vg):
...
def test_find_instances_with_all_nets(vg):
    assert vg.find_instances_with_all_nets("Top", ["n_mid", "OUT1"]) == ["u_buf0"]

def test_find_instances_with_all_nets_no_match(vg):
    assert vg.find_instances_with_all_nets("Top", ["foo", "bar"]) == []

def test_find_instances_with_all_nets_missing_module(vg):
    with pytest.raises(ValueError):
        vg.find_instances_with_all_nets("Nope", ["A"])

def test_find_instances_paths_with_all_nets(vg):
    assert vg.find_instances_paths_with_all_nets("Top", ["n_mid", "OUT1"]) == ["u_buf0"]

def test_net_to_pin_paths_for_instance_leaf(vg):
    assert vg.net_to_pin_paths_for_instance("u_buf0") == {
        "n_mid": "u_buf0/A",
        "OUT1": "u_buf0/Y",
    }

def test_net_to_pin_paths_for_instance_nested(vg):
...
def test_resolve_hier_pin_leaf_std_cell_returns_same_leaf(vg):
    assert vg.resolve_hier_pin("u_buf0/A") == ["u_buf0/A"]

def test_resolve_hier_pin_descends_into_submodule(vg):
    assert vg.resolve_hier_pin("u_leaf2/IN") == ["u_leaf2/leafbuf/A"]

def test_resolve_hier_pin_nested_submodule(vg):
    assert vg.resolve_hier_pin("u_mid/u_leaf1/IN") == ["u_mid/u_leaf1/leafbuf/A"]

def test_resolve_hier_pin_missing_target_pin(vg):
    with pytest.raises(KeyError):
        vg.resolve_hier_pin("u_buf0/ZZ")

def test_resolve_hier_pin_missing_instance(vg):
    with pytest.raises(KeyError):
        vg.resolve_hier_pin("u_mid/nope/A")

def test_resolve_hier_pin_rejects_short_path(vg):
    with pytest.raises(ValueError):
        vg.resolve_hier_pin("u_buf0")

def test_get_instance_pins_leaf(vg):
    assert vg.get_instance_pins("u_buf0") == ["A", "Y"]

def test_get_instance_pins_nested(vg):
    assert vg.get_instance_pins("u_mid/u_nand1") == ["A", "B", "Y"]

def test_get_module_instance_nets_top(vg):
...
def test_get_module_instance_nets_missing_module(vg):
    with pytest.raises(ValueError):
        vg.get_module_instance_nets("Nope")

def test_net_to_pin_paths_for_instance_resolved(vg, monkeypatch):
    monkeypatch.setattr(
        vg,
        "net_to_pin_paths_for_instance",
        lambda path: {"N1": "u0/A", "N2": "u0/B"},
    )
    monkeypatch.setattr(
        vg,
        "resolve_hier_pin",
...
def test_nearest_port_from_pin_rejects_invalid_num_ports(vg):
    with pytest.raises(ValueError):
        vg.nearest_port_from_pin("X", num_ports=0)

def test_nearest_port_from_pin_single_port(vg, monkeypatch):
    monkeypatch.setattr(
        vg,
        "path_to_nearest_target_sentinel",
        lambda pin, targets, reverse=False: (["dummy"], "OUT1"),
    )
    assert vg.nearest_port_from_pin("u_buf0/A", reverse=False, num_ports=1) == ["OUT1"]

def test_nearest_port_from_pin_single_port_none(vg, monkeypatch):
    monkeypatch.setattr(
        vg,
        "path_to_nearest_target_sentinel",
        lambda pin, targets, reverse=False: ([], None),
    )
    assert vg.nearest_port_from_pin("u_buf0/A", num_ports=1) == []

def test_nearest_port_from_pin_multiple_forward(vg):
    ...
    assert vg.nearest_port_from_pin("PIN", reverse=False, num_ports=2) == ["OUT1", "OUT2"]

def test_nearest_port_from_pin_multiple_reverse(vg):
...
def test_nearest_port_from_pin_multiple_no_ports(vg):
    vg.graph.add_edge("PIN", "N1")
    vg.output_ports = {"OUT1", "OUT2"}
    assert vg.nearest_port_from_pin("PIN", reverse=False, num_ports=2) == []

def test_nearest_ports_from_instance_pin_nets(vg, monkeypatch):
    monkeypatch.setattr(
        vg,
    ...
    def fake_nearest(pin, reverse=False, num_ports=1):
        if pin == "p1":
            return ["OUT1", "OUT2"]
        if pin == "p2":
            return ["OUT2"]
