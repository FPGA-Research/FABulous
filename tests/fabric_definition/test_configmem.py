...
def test_a_tile_carries_the_memory_it_was_built_with(tmp_path: Path) -> None:
    def build(config_mem: ConfigMem | None) -> Tile:
        return Tile(
            name="T",
            ports=[],
            bels=[],
            tile_dir=tmp_path / "T.csv",
            matrix_dir=Path(),
    ...
    assert build(None).config_mem is None
    assert build(MEMORY).config_mem is MEMORY


def test_the_parser_reads_a_tile_memory_where_one_exists(tmp_path: Path) -> None:
    tile = Tile(
        name="T",
        ports=[],
        bels=[],
    ...
    empty = ConfigMem.default(0, frame_bits_per_row=8, max_frames_per_col=3)
    assert config_mem_csv_of(tile) == tmp_path / "T_ConfigMem.csv"
    # With no file yet the tile holds the mapping generating one would write.
    assert read_config_mem_of(tile, frame_bits_per_row=8, max_frames_per_col=3) == empty
    empty.to_csv(config_mem_csv_of(tile))
    assert read_config_mem_of(tile, frame_bits_per_row=8, max_frames_per_col=3) == empty

    # A file holding bits the tile does not have is a mapping for another tile.
    MEMORY.to_csv(config_mem_csv_of(tile))
    with pytest.raises(ValueError, match="bitmask mismatch"):
        read_config_mem_of(tile, frame_bits_per_row=8, max_frames_per_col=3)
