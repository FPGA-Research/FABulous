...
`tile_map` is stored top row first (see `Tile`). `get_sub_tile_offset` uses a
top-left origin, matching `get_anchor_offset` and `get_master_offset`, while
`is_root_tile` applies a bottom-left one.
"""
...
def _leaf(
    ...
    return Tile(
        ...
        else SwitchMatrix(matrix_file=Path(), connections={}),
        userCLK=False,
    )
...
class TestPerimeterAndInternalPorts:
    ...
    def test_composite_perimeter_ports(self) -> None:
        ...
        assert top.ports_on(Side.NORTH) in ports["0,0"]
        assert top.ports_on(Side.EAST) in ports["0,0"]
        assert top.ports_on(Side.WEST) in ports["0,0"]
        assert top.ports_on(Side.SOUTH) not in ports["0,0"]
        ...
        assert bot.ports_on(Side.SOUTH) in ports["0,1"]
        assert bot.ports_on(Side.EAST) in ports["0,1"]
        assert bot.ports_on(Side.WEST) in ports["0,1"]
        assert bot.ports_on(Side.NORTH) not in ports["0,1"]
