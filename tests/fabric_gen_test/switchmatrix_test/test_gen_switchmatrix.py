...
class TestCompositeSwitchMatrixConstants:
    ...
    def _gen(
        ...
        bot = make_empty_tile(
            "DSP_bot",
            [jump_port("x", IO.OUTPUT, wire_count=1)],
            tile_dir=tmp_path,
        ...
        (composite,) = parse_composite_tiles_csv(csv_path, {"DSP_bot": bot})
...
class TestCompositeWithoutWrapperMatrix:
    """A composite with no `MATRIX` line has no wrapper matrix to generate.

    The parser leaves `matrix_dir` as `None` (previously the `Path()`
    sentinel); `genTileSwitchMatrix` must recognise that via `is None` and
    return before touching any RTL writer.
    """

    def test_generation_skips_composite_without_matrix_line(
        self, tmp_path: Path
    ) -> None:
        bot = make_empty_tile(
            "DSP_bot",
        ...
        csv_path = tmp_path / "DSP.csv"
        csv_path.write_text("SuperTILE,DSP\nDSP_bot\nEndSuperTILE\n")
        (composite,) = parse_composite_tiles_csv(csv_path, {"DSP_bot": bot})

        assert composite.matrix_dir is None
        # No writer is needed: a matrix-less composite returns before any RTL
        # is emitted (a real writer would dereference the empty muxes_dict).
        genTileSwitchMatrix(None, composite, switch_matrix_debug_signal=False)
