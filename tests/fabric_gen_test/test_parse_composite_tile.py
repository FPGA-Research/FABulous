...
A `SuperTILE` block becomes a single composite `Tile` whose `tile_map`
...
def dsp_subtiles() -> dict[str, Tile]:
    ...
    bot = make_empty_tile(
        ...
        [
            ...
            jump_port("Q", IO.INPUT, wire_count=1),
        ],
        pin_order_config={},
    )
...
class TestPlainCompositeTile:
    ...
    def test_plain_block_is_composite_with_empty_wrapper(
        ...
        (composite,) = parse_composite_tiles_csv(csv_path, dsp_subtiles)
        ...
        assert composite.switch_matrix.no_config_bits == 0
        assert composite.switch_matrix.connections == {}
    ...
    def test_subtiles_marked_part_of_supertile(
        ...
        csv_path = _write_supertile_csv(tmp_path, "DSP_top\nDSP_bot\n")
        parse_composite_tiles_csv(csv_path, dsp_subtiles)
        assert dsp_subtiles["DSP_top"].part_of_composite is True
        assert dsp_subtiles["DSP_bot"].part_of_composite is True
...
class TestWrapperCompositeTile:
    ...
    def test_wrapper_bel_matrix_and_master(
        ...
        # source). parseBelFile reads a real Verilog file, so mock it.
        ...
        assert composite.switch_matrix.no_config_bits == 1
