...
def test_supertile_edge_child_usrclk_connects_to_global(
    ...
    The UserCLK chain flows downward from the top of the fabric, so the cell at
    the fabric's top edge (no tile above) must wire its ``UserCLK`` port to the
    global ``UserCLK`` signal, not to a phantom ``Tile_X*Y*_UserCLKo`` that is
    never driven.

    The old code used ``y + 1`` (anchor + 1) to decide whether to fall back to
    the global clock, but for a child cell the correct check is ``y + j + 1``;
    the wrong check emitted ``Tile_X0Y2_UserCLKo`` — a wire that does not exist —
    triggering the OpenROAD GRT-0010 error.

    The composite wrapper names its cells top row first, so the top-edge cell
    (``ST_top``) is ``Tile_X0Y0`` within the wrapper; its UserCLK port carries
    the global clock.
    """
    top = mk_tile("ST_top")
    bot = mk_tile("ST_bot")
    ...
    # The top-edge cell (ST_top) is Tile_X0Y0 within the wrapper; its UserCLK
    # port must connect to the global UserCLK signal.
    assert ".Tile_X0Y0_UserCLK(UserCLK)" in rtl
